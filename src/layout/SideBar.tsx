  ...
  return (
    ...
    >
      <div className="flex flex-col gap-16 items-start">
        ...
        <ul className="flex flex-col gap-8 text-grey-300">
          {pages.map((p) => (
            <motion.li
              key={p.id}
              className={`hover:text-white  ${!isSideBarOpen ? '-ml-6' : ''}`}
            >
              <NavLink
                to={p.to}
                className={({ isActive }) =>
      ...
      <div className="flex flex-col gap-4">
        <button
          className={`flex gap-1 items-center p-2 cursor-pointer text-grey-300 hover:text-white rounded-lg transition-colors duration-200 ${isSideBarOpen ? '' : 'border-none'} `}
          onClick={logout}
        >
          <motion.div
            className={!isSideBarOpen ? '-ml-2' : 'px-4 -ml-8'}
          ...
          <motion.span
            ...
            transition={{ duration: 0.2 }}
            className="text-grey-300 overflow-hidden whitespace-nowrap hover:text-white"
          >
            Logout
          </motion.span>
        </button>
