        ...
        {pages.map((p) => (
          <li key={p.id} className="flex items-end mt-2">
            <NavLink
              to={p.to}
              className={({ isActive }) =>
                `flex items-center px-5.5 md:px-6 pt-2 md:flex-col h-full gap-1 font3 transition-colors duration-200 ${isActive ? "bg-white rounded-t-lg border-b-2 border-green" : ""}`
              }
            >
              {({ isActive }) => (
                <>
